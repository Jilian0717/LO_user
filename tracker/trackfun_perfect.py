...
#jx for perfect restart
def get_fn_list(idt, Ldir):
    dd = idt.strftime('%Y.%m.%d')
    indir = Ldir['roms_out'] / Ldir['gtagex'] / ('f' + dd)
    fn_list = sorted(indir.glob('ocean_his_*'))
    this_dt = datetime.strptime(dd, Lfun.ds_fmt)
    
    if 'ocean_his_0001.nc' != str(fn_list[0])[-17:]:
        this_dt0 = (this_dt - timedelta(days=1))
        fn_prev =  Ldir['roms_out'] / Ldir['gtagex'] / ('f' + this_dt0.strftime(Lfun.ds_fmt)) / 'ocean_his_0025.nc'
        fn_list = np.append(fn_prev,fn_list)   # append ocean_his_0025.nc from previous day to [ocean_his_0002.nc to ocean_his_0025.nc]  
    return fn_list
