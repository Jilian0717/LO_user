...
#%------------------------------------------------
Ldir = Lfun.Lstart()
Ldir['roms_out'] = Ldir['roms_out4']
Ldir['gtagex'] = 'cas7_trapsV00_meV00'
...
Oxy_pro_spatial = np.zeros((24*366, NX, NY))
Oxy_nitri_spatial = np.zeros((24*366, NX, NY))
Oxy_remi_spatial = np.zeros((24*366, NX, NY))
Oxy_sed_spatial = np.zeros((24*366, NX, NY))
Oxy_sed_spatial2 = np.zeros((24*366, NX, NY)) # the 2nd way to calculate SOD
cnt = 0
...
#
        ...
        #Oxy_vol = Oxy * vol # only account for Salish Sea
        ...
        if np.nanmin(0.43*swrad) > 0: 
            for k in np.arange(29,-1,-1): # surface to bottom                
        ...
        Oxy_pro_sum.append(np.nansum(Oxy_pro[:,jj,ii])) # only account for Salish Sea
        ...
        Oxy_sed_spatial[cnt,:,:] = F_NH4 * 106/16 * area * inDomain

        #---------- sediment SOD: 2nd way (following fennel.h) ----------
        # SdetritusN decomposition in sediment
        # FC_S = (SdetN_bot * Ws_S)/24 # mmol/m2/hr
        # cff1_S = FC_S / dz[0,:,:] * 1  # mmol/m3 
        ...
        Oxy2 = Oxy[0,:,:] - NH4_gain_S*106/16  #? bottom oxygen loss from decomposing SdetN
        # FC_L = (LdetN_bot * Ws_L)/24 # mmol/m2/hr
        # cff1_L = FC_L / dz[0,:,:] * 1  # mmol/m3 
        cff1_L = (LDeN[0,:,:] * 80)/24 / dz[0,:,:] * 1  # mmol/m3     
        NH4_gain_L = np.zeros([NX,NY])        
        for i in range(NX): # loop the whole domain
            for j in range(NY):
                if cff1_L[i,j]*106/16 <= Oxy2[i,j]:  # use O2: PN + O2 --> NH4;                                    
